...
from visual import color, display, curve, sphere, label # http://www.vpython.org/webdoc/visual/
...
CUBE_SIZE          = 1.              # container is a cube of CUBE_SIZE sides
...
PARTICLES_COLORS   = [color.red, color.blue, color.yellow, color.cyan, color.magenta, color.green]
...
# Window
window = display( title      = WINDOW_TITLE,
                  autocenter = 1,
                  autoscale  = 1,
                  width      = WINDOW_WIDTH,
...
# Axis X
axisX  = curve( pos = [ (-sigma, 0, 0), ( sigma, 0, 0) ], color=color.white, radius=CUBE_THICKNESS)


# Axis Y
axisY  = curve( pos = [ (0, -4*sigma, 0), (0, 4*sigma, 0) ], color=color.white, radius=CUBE_THICKNESS)
...
for i in range( PARTICLES_TOTAL ):
	...
	particle = sphere( pos    = (x,y,z),
	                   radius = PARTICLES_SIZE,
	                   color  = PARTICLES_COLORS[i % 6]
	                 )
...
while True:
	...
	for i in range( PARTICLES_TOTAL ):
		...
		particles[i].radius    = newThickness * 2.
