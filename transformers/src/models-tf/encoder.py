...
import torch
import unittest
# import neural network package from torch
...
class TestTransformerEncoder(unittest.TestCase):
    def test_transformer_encoder_single_sequence_batch(self):
        ...
        with torch.no_grad():
            # Initialize a transformer encoder (qkv_dim is automatically
            # set to hidden_dim // num_heads)
    ...
    def test_transformer_encoder_multi_sequence_batch(self):
        ...
        # Initialize a transformer encoder (qkv_dim is automatically set to
        ...
        with torch.no_grad():
            ...
            encoder.eval()
            input_batch = torch.IntTensor(
                en_vocab.batch_encode(
                    batch, add_special_tokens=False,
                    padding=True)
            )
            src_padding_mask = input_batch != en_vocab.token2index[
                en_vocab.PAD]
