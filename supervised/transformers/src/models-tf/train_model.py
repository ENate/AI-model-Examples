    ...
    for e in range(n_epochs):
        for i, (src_batch, src_mask, tgt_batch, tgt_mask) in enumerate(
            ...
            encoder_output = transformer.encoder(
                src_batch,
                src_padding_mask=src_mask)  # type: ignore

            # Perform one decoder forward pass to obtain *all* next-token
            # predictions for every index i given its
            # previous *gold standard* tokens [1,..., i] (i.e. teacher forcing)
            # in parallel/at once.
            ...
            # Align labels with predictions: the last decoder prediction is
            # meaningless because we have no target token
            # for it. The BOS token in the target is also not something we
            # want to compute a loss for.
            ...
            # Compute the average cross-entropy loss over all next-token
            # predictions at each index i given [1, ..., i]
            # for the entire batch. Note that the original paper uses label
            # smoothing (I was too lazy).
            batch_loss = criterion(
                decoder_output.contiguous().permute(0, 2, 1),
                tgt_batch.contiguous().long(),
            )

            # Rough estimate of per-token accuracy in the current
            # training batch
            batch_accuracy = (
...
class TestTransformerTraining(unittest.TestCase):
    ...
    def test_copy_task(self):
        """
        Test training by trying to (over)fit a simple copy dataset - bringing
        the loss to ~zero. (GPU required)
        """
        device = (
            torch.device("cuda")
        ...
        # Construct vocabulary and create synthetic data by uniform randomly
        # sampling tokens from it
        # Note: the original paper uses byte pair encodings, we simply take
        # each word to be a token.
        ...
        # Construct src-tgt aligned input batches (note: the original paper
        # uses dynamic batching based on tokens)
        ...
        # Initialize learning rate scheduler, optimizer and loss (note: the
        # original paper uses label smoothing)
        ...
        # Start training and verify ~zero loss and >90% accuracy on the
        # last batch
